...
def agent_system_prompt() -> str:
    return (
        ...
        "Use the provided tool_registry as the execution contract: read-only tools inspect files, source semantics, diffs, checks, browser proof, and safe diagnostics; apply_patch_to_draft/write_file mutating tools are serialized through the draft edit validator. "
        ...
        "All writes must be explicit draft actions or mutating draft tool requests in the action payload. "
        "Use hunk patches for focused edits and full-file create/replace only when creating or substantially rewriting a file. "
        "Return only the action payload requested by the schema."
    )
