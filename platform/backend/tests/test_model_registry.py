...
def test_generation_modes_route_agent_turn_to_distinct_default_profiles() -> None:
    ...
    assert {FAST_CODE_MODEL, BALANCED_CODE_MODEL, QUALITY_CODE_MODEL} == {CODEX_MAX_MODEL}
...
def test_code_agent_profiles_route_all_generation_defaults_to_codex_max() -> None:
    routed_agent_models = {str(profile["routing"]["agent_turn"]) for profile in TASK_PROFILES.values()}
    assert routed_agent_models == {CODEX_MAX_MODEL}
